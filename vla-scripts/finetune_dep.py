...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    vla = OpenVLAForActionPrediction.from_pretrained(cfg.vla_path, config=config, trust_remote_code=True)

    image_processor = PrismaticImageProcessor.from_pretrained(cfg.vla_path)

# 加载 tokenizer（照常）
    tokenizer = AutoTokenizer.from_pretrained(cfg.vla_path, trust_remote_code=True)
...
# 用你本地两个组件构造 processor
    ...
    # image_processor = AutoImageProcessor.from_pretrained(cfg.vla_path, trust_remote_code=True)
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if should_save_ckpt:
                if distributed_state.is_main_process:
                    print(f"[INFO] Saving checkpoint at step {gradient_step_idx}")

                    # 保存 processor（无论是否 LoRA）
                    processor.save_pretrained(run_dir)

                    if cfg.use_lora:
                        print("[INFO] Merging LoRA into base model for saving...")
                        base_vla = OpenVLAForActionPrediction.from_pretrained(cfg.vla_path, config=config, trust_remote_code=True)
                        merged_vla = PeftModel.from_pretrained(base_vla, adapter_dir)
                        merged_vla = merged_vla.merge_and_unload()
                    else:
                        merged_vla = vla.module  # 没有 LoRA，直接保存
                    ...
                    print(f"[✅] Saved full checkpoint at: {checkpoint_dir}")
