...
class PrismaticProjector(nn.Module):
    def __init__(self, use_fused_vision_backbone: bool, vision_dim: int, llm_dim: int, depth_dim: int) -> None:
        ...
        # 映射深度特征到同一维度
        self.depth_fc = nn.Linear(self.depth_dim, self.llm_dim, bias=True)
        self.act = nn.GELU()
...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    def __init__(self, config: PrismaticConfig) -> None:
        ...
        self.depth_backbone = DepthFeatureExtractor()
        # Create Multimodal Projector
        self.projector = PrismaticProjector(
            vision_dim=self.vision_backbone.embed_dim,
            depth_dim=self.depth_backbone.embed_dim,
    ...
    def forward(
        ...
        # === Handle Multimodal Forward ===
            ...
            if depth_pixel_values is not None:
                depth_features = self.depth_backbone(depth_pixel_values)
            else:
                depth_features = None
            # Projection Logic =>> Update Attention Mask
            projected_patch_embeddings = self.projector(patch_features, depth_features)
