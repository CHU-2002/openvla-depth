...
from prismatic.models.crossattention import CrossAttention, CrossAttentionLora
...
class PrismaticVisionBackbone(nn.Module):
    def __init__(
        ...
        self.expand_patch_embed_3_to_6(self.featurizer.patch_embed)
    ...
    def expand_patch_embed_3_to_6(self, patch_embed: nn.Module):
        """把 ViT patch_embed.proj 从 3→6 通道，并迁移权重."""
        old_conv = patch_embed.proj                      # Conv2d(3, E, k, k)
        E, _, k, _ = old_conv.weight.shape
        ...
        # 复制权重：前 3 通道 ← 旧 RGB；后 3 通道 ← 平均
        with torch.no_grad():
            new_conv.weight[:, :3] = old_conv.weight
            # 将 HHA 通道初始化为 RGB 权重的平均
            new_conv.weight[:, 3:] = old_conv.weight.mean(dim=1, keepdim=True)
            if old_conv.bias is not None:
                new_conv.bias[:] = old_conv.bias

        # 用新卷积替换旧的 patch_embed.proj
        patch_embed.proj = new_conv


    def forward(self, pixel_values: torch.Tensor, depth_values: torch.Tensor) -> torch.Tensor:
        ...
        rgbd = torch.cat([img, depth_values], dim=1)  # [bsz,6,H,W]
        patches, patches_fused = self.featurizer(rgbd), self.fused_featurizer(img_fused)
...
#     """Project concatenated RGB **and** depth patch‑features into the LLM embedding space.

#     Args:
#         vision_dim (int):     Total *RGB* embedding dim after (optional) fused‑backbone concatenation.
#         depth_dim  (int):     Depth‑patch embedding dim.
#         llm_dim    (int):     Target embedding dim expected by the LLM.
#         use_fused_vision_backbone (bool):  If True, keeps the authors’ wider‑MLP pathway.
#     """
...
#         self.use_fused_vision_backbone = use_fused_vision_backbone
#         self.in_dim = vision_dim + depth_dim
#         self.llm_dim = llm_dim

#         if not self.use_fused_vision_backbone:
#             # ---------- 2‑layer “narrow” MLP ----------
#             self.fc1 = nn.Linear(self.in_dim, self.llm_dim, bias=True)
#             self.act1 = nn.GELU()
#             self.fc2 = nn.Linear(self.llm_dim, self.llm_dim, bias=True)
#         else:
#             # ---------- 3‑layer “wider” MLP (×4 inflation) ----------
#             hidden_dim = 4 * self.in_dim
#             self.fc1 = nn.Linear(self.in_dim, hidden_dim, bias=True)
#             self.act1 = nn.GELU()
#             self.fc2 = nn.Linear(hidden_dim, self.llm_dim, bias=True)
#             self.act2 = nn.GELU()
#             self.fc3 = nn.Linear(self.llm_dim, self.llm_dim, bias=True)

#     def _align_patches(self, t: torch.Tensor, target_len: int) -> torch.Tensor:
#         """
#         t: [B, N, C]   -- N 可能不是完全平方数
#         target_len:    -- 对齐输出的 patch 数 (通常 = max(N_rgb, N_depth))
#         返回: [B, target_len, C]
#         """
#         B, N, C = t.shape
#         if N == target_len:
...
#         W = int(math.ceil(N / H))                    # H*W >= N，且尽量接近方阵
#         if H * W < N:
#             H += 1                                   # double‑check

#         # ② 若 H*W > N，序列末尾 pad 0-token 以凑满
#         pad_len = H * W - N
#         if pad_len > 0:
#             t = F.pad(t, (0, 0, 0, pad_len), value=0.0)   # [B, N+pad, C]

#         # ③ reshape → interpolate → flatten
#         t = t.transpose(1, 2).reshape(B, C, H, W)          # [B,C,H,W]

#         tgt_hw = int(math.sqrt(target_len))
#         t = F.interpolate(t, size=(tgt_hw, tgt_hw),
#                         mode="bilinear", align_corners=False)

#         t = t.flatten(2).transpose(1, 2)                   # [B,target_len,C]
#         return t


#     def forward(self, rgb_patches: torch.Tensor, depth_patches: torch.Tensor) -> torch.Tensor:
#         """Concatenate **[B, N, C_rgb]** & **[B, N, C_depth] → [B, N, C_rgb+C_depth]**, then project."""

#         if rgb_patches.shape[1] != depth_patches.shape[1]:
#             tgt = max(rgb_patches.shape[1], depth_patches.shape[1])
#             rgb_patches   = self._align_patches(rgb_patches,   tgt)
#             depth_patches = self._align_patches(depth_patches, tgt)
#         x = torch.cat([rgb_patches, depth_patches], dim=2)  # [B, N, C_total]
...
# class PrismaticProjector(nn.Module):
#     def __init__(self, use_fused_vision_backbone: bool, vision_dim: int, llm_dim: int, depth_dim: int) -> None:
#         super().__init__()
#         self.use_fused_vision_backbone = use_fused_vision_backbone
#         self.vision_dim, self.llm_dim = vision_dim, llm_dim
...
#         # 映射深度特征到同一维度
#         self.depth_fc = nn.Linear(depth_dim, self.llm_dim, bias=True)

#         # 交叉注意力：用深度特征增强 RGB
#         self.cross_attn = CrossAttentionLora(embed_dim=self.llm_dim, num_heads=8)

#     def forward(self, img_patches: torch.Tensor, depth_patches: torch.Tensor) -> torch.Tensor:
#         """
#         img_patches: [B, N, vision_dim]
#         depth_patches: [B, M, depth_dim]
#         """
#         # Step 1: 投影 RGB 特征
...
#         # Step 2: 投影深度特征
#         d = self.depth_fc(depth_patches)

#         # Step 3: Cross-Attention，用深度信息增强 RGB
#         x_enhanced = self.cross_attn(rgb_features=x, depth_features=d)
...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def forward(
        ...
        # === Handle Multimodal Forward ===
            ...
            patch_features = self.vision_backbone(pixel_values,depth_pixel_values)
