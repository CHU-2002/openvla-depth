...
def normalize_action_and_proprio(traj: Dict, metadata: Dict, normalization_type: NormalizationType):
    ...
    elif normalization_type == NormalizationType.No:
        return traj
...
def relabel_ur5_actions(traj: Dict[str, Any]) -> Dict[str, Any]:
    """
    Relabels actions so that the first 6 dims are the EEF pose difference (i -> i+1),
    and the last dim is the 'gripper' state of the *next* frame (i+1).
    Discards the last timestep for everything else, but discards the *first* frame
    for the gripper dimension to align with the difference.
    """
    movement_actions = traj["observation"]["pose"][1:, :6] - traj["observation"]["pose"][:-1, :6]
    traj_truncated = tf.nest.map_structure(lambda x: x[:-1], traj)
    gripper_next = traj["action"][1:, -1:]  # shape = [T-1, 1]
    traj_truncated["action"] = tf.concat([movement_actions, gripper_next], axis=1)
