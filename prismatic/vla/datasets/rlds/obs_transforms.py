...
def decode_and_resize(
    ...
    #     if depth.dtype == tf.string:
    #         if tf.strings.length(depth) == 0:
    #             h, w = depth_resize_size.get(name, (224, 224))
    #             depth = tf.zeros((h, w, 1), dtype=tf.float32)
    #         else:
    #             depth = tf.io.decode_png(depth, channels=1, dtype=tf.uint16)
    #             depth = tf.cast(depth, tf.float32)
    #     elif depth.dtype == tf.uint16:
    #         depth = tf.cast(depth, tf.float32)
    #     elif depth.dtype != tf.float32:
    #         raise ValueError(f"Unsupported depth dtype: found depth_{name} with dtype {depth.dtype}")
    ...
    #     # ✅ 最终统一成 (H, W)
    #     depth = tf.squeeze(depth, axis=-1) if depth.shape.rank == 3 else depth
