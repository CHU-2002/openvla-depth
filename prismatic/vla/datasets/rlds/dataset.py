...
def print_tensor_shapes(d, prefix=''):
    for k, v in d.items():
        if isinstance(v, dict):
            ...
            print(f"{prefix}{k}: {getattr(v, 'shape', 'not a tensor')}")
def fix_obs_shape(obs):
    if "image_primary" in obs:
        img = obs["image_primary"][0]
        img = tf.ensure_shape(img, [224, 224, 3])
        obs["image_primary"] = tf.stack([img])
    if "depth_primary" in obs:
        depth = obs["depth_primary"][0]
        depth = tf.ensure_shape(depth, [224, 224, 1])
        obs["depth_primary"] = tf.stack([depth])
    return obs
...
def apply_frame_transforms(
    ...
    *,
    ...
    def apply_obs_transform(fn: Callable[[Dict], Dict], frame: Dict) -> Dict:
        print("frame['observation']:", frame["observation"])
        print("shapes:", tf.nest.map_structure(lambda x: tf.shape(x), frame["observation"]))
